...
export const createGradeFormat: express.RequestHandler = async (req: Request, res: Response) => {
  try {
    ...
    // Fetch existing grade formats for the school
    const existingGradeFormats: IGradeFormat[] = await GradeFormat.find({ school: schoolId });
    ...
    for (const format of gradeFormats) {
      ...
      const isOverlapping = existingGradeFormats.some(existingFormat => {
        const existingMinScore = existingFormat.miniScore;
        const existingMaxScore = existingFormat.maxScore;

        // Check if the new range overlaps with any existing grade range
        return (
          (miniScore >= existingMinScore && miniScore <= existingMaxScore) ||
          (maxScore >= existingMinScore && maxScore <= existingMaxScore) ||
          (miniScore <= existingMinScore && maxScore >= existingMaxScore)
        );
      });

      if (isOverlapping) {
        return res.status(400).json({ error: 'Grade ranges overlap with existing grades' });
      }
