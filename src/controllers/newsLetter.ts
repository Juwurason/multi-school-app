...
export const sendNewsLetter: express.RequestHandler = async (req, res) => {
  try {
    ...
    await newsLetter.updateOne({ _id: newsLetter._id }, { status: 'sent' });

    return res.status(200).json({ message: 'Newsletters saved and sent successfully' });
  } catch (error) {
    console.error('Error saving and sending newsletters:', error);
    return res.status(500).json({ error: 'Internal server error' });
  }
};
